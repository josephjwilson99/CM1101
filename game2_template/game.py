...
def execute_take(item_id):
    ...
    if item_id in current_room["items"]:
        loop_counter = 0
        total_mass = 0
        for inv_items in inventory:
            total_mass = total_mass + inventory[loop_counter]["mass"]
            loop_counter = loop_counter + 1
        new_mass = total_mass + item_id["mass"]
        if new_mass > 7:
            print("You are over encumbered. Drop something.")
        else:
